...
IDENTIFIED BY '#{config['password']}' WITH GRANT OPTION;
  ...
  task :environment do
    CAMINIO_ENV = ENV['CAMINIO_ENV'] || 'development'
    MIGRATIONS_DIR = ENV['MIGRATIONS_DIR'] || 'db/migrate'
  end

  task :configuration => :environment do
    @config = YAML.load_file( "#{Dir.pwd}/config/database.yml" )[CAMINIO_ENV]
  end
  ...
  desc 'Create the database from config/database.yml for the current CAMINIO_ENV'
  task :create => :configure_connection do
    create_database @config
  end

  desc 'Drops the database for the current CAMINIO_ENV'
